...
func NewHandlerDecoration(verb Verb, handler interface{}) *HandlerDecoration {
	...
	if handlerType.NumIn() != 1 {
		log.Panic("handler must have only 1 parameter")
	}
	if handlerType.In(0) != contextType {
		log.Panic("the parameter of handler must be context.Context")
	}
	returnValueCount := handlerType.NumOut()
	// check return values count
	if count, ok := verbMapping[verb]; ok {
		if count != returnValueCount {
			log.Panicf("%s handler must have %d return value", verb, count)
		}
	} else {
		log.Panicf("unknown verb: %s", verb)
	}
	// check the last return value
	if !handlerType.Out(returnValueCount - 1).AssignableTo(errorType) {
		log.Panicf("the last return value of %s handler must be compatible with error: %s",
...
func (hd *HandlerDecoration) Handle(request *restful.Request, resp *restful.Response) {
	...
	if errValue.IsNil() {
		...
		default:
			...
			log.Fatalf("app enters unknown area. handler should not have %d results", len(result))
		}
	}
	// handle error
	...
	default:
		// should not come here
		log.Fatalf("%s handler returns an unknown error type, check the function: %s",
